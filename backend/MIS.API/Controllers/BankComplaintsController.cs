...
public sealed class BankComplaintsController(IBankComplaintService service) : ControllerBase
{
    ...
    public async Task<ActionResult<BankComplaintDetailsDto>> Upload(Guid bankId, Guid complaintId, [FromForm] IFormFile file, CancellationToken token) { if (file is null || file.Length == 0) return BadRequest(ApiErrorResponse.Failure("A non-empty attachment is required.")); await using var stream = file.OpenReadStream(); return await service.UploadAttachmentAsync(bankId, complaintId, file.FileName, file.ContentType ?? "application/octet-stream", file.Length, stream, token); }

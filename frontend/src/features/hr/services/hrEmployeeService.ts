...
export interface EmployeeQuery { page: number; pageSize: number; search: string; departmentId: string; status: string; }

export const hrEmployeeService = {
  async getEmployees(query: EmployeeQuery): Promise<PagedEmployees> {
    const { data } = await apiClient.get<PagedEmployees>('/hr/employees', { params: { page: query.page, pageSize: query.pageSize, search: query.search || undefined, departmentId: query.departmentId || undefined, status: query.status } });
    return data;
  },

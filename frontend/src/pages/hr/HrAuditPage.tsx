      ...
      <Card padding="none">
        <div className="grid gap-4 border-b border-slate-200 bg-slate-50/60 p-4 md:grid-cols-2 lg:p-5">
          ...
          <div className="grid gap-3 md:col-span-2 sm:grid-cols-2 xl:grid-cols-4">
            <label><span className="mb-2 flex items-center gap-2 text-sm font-bold text-slate-700"><Filter className="h-4 w-4 text-mis-primary" />{language === 'ar' ? 'الإجراء' : 'Action'}</span><select aria-label={text.allActions} className="h-11 w-full rounded-xl border border-slate-300 bg-white px-3 text-sm shadow-sm" onChange={(event) => setQuery((current) => ({ ...current, action: event.target.value, page: 1 }))} value={query.action}><option value="">{text.allActions}</option>{actionOptions.map((action) => <option key={action} value={action}>{labelAction(action, language)}</option>)}</select></label>
            <label><span className="mb-2 flex items-center gap-2 text-sm font-bold text-slate-700"><Filter className="h-4 w-4 text-mis-primary" />{language === 'ar' ? 'نوع السجل' : 'Record type'}</span><select aria-label={text.allEntities} className="h-11 w-full rounded-xl border border-slate-300 bg-white px-3 text-sm shadow-sm" onChange={(event) => setQuery((current) => ({ ...current, entityType: event.target.value, page: 1 }))} value={query.entityType}><option value="">{text.allEntities}</option>{entityOptions.map((entity) => <option key={entity} value={entity}>{labelEntity(entity, language)}</option>)}</select></label>
            <label><span className="mb-2 flex items-center gap-2 text-sm font-bold text-slate-700"><CalendarRange className="h-4 w-4 text-mis-primary" />{text.from}</span><input aria-label={text.from} className="h-11 w-full rounded-xl border border-slate-300 bg-white px-3 text-sm shadow-sm" onChange={(event) => setQuery((current) => ({ ...current, from: event.target.value, page: 1 }))} type="date" value={query.from} /></label>
            <label><span className="mb-2 flex items-center gap-2 text-sm font-bold text-slate-700"><CalendarRange className="h-4 w-4 text-mis-primary" />{text.to}</span><input aria-label={text.to} className="h-11 w-full rounded-xl border border-slate-300 bg-white px-3 text-sm shadow-sm" min={query.from || undefined} onChange={(event) => setQuery((current) => ({ ...current, to: event.target.value, page: 1 }))} type="date" value={query.to} /></label>
          </div>
        </div>

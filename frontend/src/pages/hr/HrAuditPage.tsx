import { Activity, ChevronRight, Search, UserRound } from 'lucide-react';
...
const actionOptions = [
  ...
  'AbsenceCreated', 'AbsenceUpdated', 'AbsenceDeleted', 'DocumentUploaded', 'DocumentUpdated', 'DocumentReplaced', 'DocumentDeleted',
...
const arabicActions: Record<string, string> = {
  ...
  AbsenceCreated: 'تسجيل غياب', AbsenceUpdated: 'تحديث الغياب', AbsenceDeleted: 'حذف الغياب', DocumentUploaded: 'رفع مستند', DocumentUpdated: 'تحديث المستند', DocumentReplaced: 'استبدال المستند', DocumentDeleted: 'حذف المستند',
...
export function HrAuditPage() {
  ...
  return (
    <div className="mx-auto max-w-[1400px]">
      ...
      <Card padding="none">
        <div className="grid gap-3 border-b border-mis-border p-4 md:grid-cols-2 xl:grid-cols-[minmax(230px,1fr)_220px_185px_175px_145px_145px]">
          <label className="relative"><Search className="absolute start-3 top-3 h-5 w-5 text-slate-400" /><input className="h-11 w-full rounded-xl border border-mis-border pe-3 ps-10 text-sm" onChange={(event) => setSearch(event.target.value)} placeholder={text.search} value={search} /></label>
          ...
          <select aria-label={text.allActions} className="h-11 rounded-xl border border-mis-border bg-white px-3 text-sm" onChange={(event) => setQuery((current) => ({ ...current, action: event.target.value, page: 1 }))} value={query.action}><option value="">{text.allActions}</option>{actionOptions.map((action) => <option key={action} value={action}>{labelAction(action, language)}</option>)}</select>
          <select aria-label={text.allEntities} className="h-11 rounded-xl border border-mis-border bg-white px-3 text-sm" onChange={(event) => setQuery((current) => ({ ...current, entityType: event.target.value, page: 1 }))} value={query.entityType}><option value="">{text.allEntities}</option>{entityOptions.map((entity) => <option key={entity} value={entity}>{labelEntity(entity, language)}</option>)}</select>
          <input aria-label={text.from} className="h-11 rounded-xl border border-mis-border px-3 text-sm" onChange={(event) => setQuery((current) => ({ ...current, from: event.target.value, page: 1 }))} type="date" value={query.from} />
          <input aria-label={text.to} className="h-11 rounded-xl border border-mis-border px-3 text-sm" min={query.from || undefined} onChange={(event) => setQuery((current) => ({ ...current, to: event.target.value, page: 1 }))} type="date" value={query.to} />
        </div>

  ...
  return (
    <Modal closeLabel={t('close')} closeOnBackdrop={!saving} closeOnEscape={!saving} footer={<><Button disabled={saving} fullWidth={false} onClick={onClose} size="md" type="button" variant="outline">{t('cancel')}</Button><Button form={formId} fullWidth={false} isLoading={saving} size="md" type="submit">{t('saveChanges')}</Button></>} hideCloseButton={saving} onClose={onClose} open size="lg" title={t(record ? 'editAttendance' : 'addManualAttendance')}>
      <form className="grid gap-5 sm:grid-cols-2" id={formId} onSubmit={submit}>
        ...
        <label className="space-y-2 text-sm font-semibold text-slate-700"><span className="block">{t('checkIn')}</span><input className="h-12 w-full rounded-form border border-mis-border bg-white px-4 text-sm outline-none focus:border-mis-blue focus:shadow-input" onChange={(event) => setForm((current) => ({ ...current, checkIn: event.target.value || null }))} type="datetime-local" value={form.checkIn ?? ''} /></label>
        <label className="space-y-2 text-sm font-semibold text-slate-700"><span className="block">{t('checkOut')}</span><input className="h-12 w-full rounded-form border border-mis-border bg-white px-4 text-sm outline-none focus:border-mis-blue focus:shadow-input" onChange={(event) => setForm((current) => ({ ...current, checkOut: event.target.value || null }))} type="datetime-local" value={form.checkOut ?? ''} /></label>
  ...
  return <div className="mx-auto max-w-[1500px]">
    ...
    <section className="overflow-hidden rounded-2xl border border-mis-border bg-white shadow-sm">
      <div className="grid gap-3 border-b border-mis-border p-4 sm:grid-cols-2 xl:grid-cols-[minmax(220px,1fr)_180px_160px_160px_150px_170px_auto]">
        ...
        <select aria-label={t('department')} className="h-11 rounded-xl border border-mis-border bg-white px-3 text-sm" onChange={(event) => { setDepartmentId(event.target.value); setPage(1); }} value={departmentId}><option value="">{t('allDepartments')}</option>{departments.map((item) => <option key={item.id} value={item.id}>{language === 'ar' && item.nameArabic ? item.nameArabic : item.nameEnglish}</option>)}</select>
        <input aria-label={t('dateFrom')} className="h-11 rounded-xl border border-mis-border px-3 text-sm" onChange={(event) => { setDateFrom(event.target.value); setPage(1); }} type="date" value={dateFrom} />
        <input aria-label={t('dateTo')} className="h-11 rounded-xl border border-mis-border px-3 text-sm" onChange={(event) => { setDateTo(event.target.value); setPage(1); }} type="date" value={dateTo} />
        <select aria-label={t('attendanceStatus')} className="h-11 rounded-xl border border-mis-border bg-white px-3 text-sm" onChange={(event) => { setStatus(event.target.value as AttendanceStatus | ''); setPage(1); }} value={status}><option value="">{t('allStatuses')}</option>{attendanceStatuses.map((item) => <option key={item} value={item}>{t(statusLabels[item])}</option>)}</select>
        <select aria-label={t('attendanceSource')} className="h-11 rounded-xl border border-mis-border bg-white px-3 text-sm" onChange={(event) => { setSource(event.target.value as AttendanceSource | ''); setPage(1); }} value={source}><option value="">{t('allSources')}</option>{attendanceSources.map((item) => <option key={item} value={item}>{t(sourceLabels[item])}</option>)}</select>

...
export function ModuleLayoutShell(props: ModuleLayoutShellProps) {
  ...
  return <div className="module-shell min-h-dvh bg-mis-surface text-mis-ink" data-sidebar-collapsed={collapsed} data-module-theme={props.theme}>
    ...
    <aside aria-hidden={!isDesktop && !open} aria-label={props.navigationLabel} inert={!isDesktop && !open ? true : undefined} style={{ insetInlineStart: 0, borderInlineEndWidth: 1 }} className={`module-sidebar fixed inset-y-0 z-40 flex h-dvh w-[min(18rem,calc(100vw-3rem))] flex-col overflow-hidden border-mis-border bg-white shadow-panel transition-[width,transform] duration-200 ease-out lg:translate-x-0 lg:shadow-sm ${open ? 'translate-x-0' : props.isRtl ? 'translate-x-full' : '-translate-x-full'}`}>

import { FileText, Gauge, Languages, LogOut, Menu, ScrollText, UserRoundX, X, Database, UsersRound } from 'lucide-react';
...
export function HrLayout() {
  ...
  return (
    <div className="min-h-screen bg-mis-surface text-mis-ink">
      {open && <button aria-label="Close navigation" className="fixed inset-0 z-30 bg-mis-ink/40 lg:hidden" onClick={() => setOpen(false)} type="button" />}
      <aside style={{ insetInlineStart: 0, borderInlineEndWidth: 1 }} className={`fixed inset-y-0 z-40 flex w-72 flex-col border-mis-border bg-white transition-transform lg:translate-x-0 ${open ? 'translate-x-0' : isRtl ? 'translate-x-full' : '-translate-x-full'}`}>
        <div className="flex h-24 items-center justify-between border-b border-mis-border px-6">
          <img src={misLogo} alt="MIS Collection Firm" className="h-16 w-auto" />
          <button aria-label="Close navigation" className="rounded-lg p-2 text-slate-500 lg:hidden" onClick={() => setOpen(false)}><X /></button>
        </div>
        ...
        <nav className="flex-1 space-y-1 px-4 py-3" aria-label="HR navigation">
        ...
        <div className="border-t border-mis-border p-4">
          <div className="mb-3 px-3"><p className="truncate text-sm font-semibold text-mis-navy">{user?.fullName}</p><p className="text-xs text-slate-500">HR</p></div>
      ...
      <div style={{ [isRtl ? 'paddingRight' : 'paddingLeft']: 'var(--hr-sidebar-space)' }} className="hr-content">
        <header className="sticky top-0 z-20 flex h-20 items-center border-b border-mis-border bg-white/95 px-5 backdrop-blur sm:px-8">
          <button aria-label="Open navigation" className="me-4 rounded-lg border border-mis-border p-2 text-mis-navy lg:hidden" onClick={() => setOpen(true)} type="button"><Menu /></button>
